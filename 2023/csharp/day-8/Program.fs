...
let dirs, nodes_str = ReadInputLines "input.txt" |> Array.toList |> function 
...
let GetTotalSteps() = (loops * (uint32)max_step) + (uint32)step_nr

while current_nodes.Where(fun n -> n.Value.EndsWith('Z')).Count() <> current_nodes.Count() do
